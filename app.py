...
def extraction_node(state: SDRState) -> SDRState:
    """Manual form ki jagah — LinkedIn URL ya company website se lead fields auto-fill karta hai."""
    ...
    try:
        data = {"q": f"site:{url}" if is_linkedin else f"{url} about company", "num": 5}
    ...
    if not is_linkedin:
        try:
            ...
            body = " ".join(p.get_text() for p in soup.find_all("p"))[:1200]
            page_text = f"{meta_text}\n{body}"
        except Exception:
            pass

    llm = ChatGroq(api_key=state["groq_key"], model="openai/gpt-oss-120b")
    prompt = f"""Extract lead info from the context below for a B2B SDR tool.
Return ONLY valid JSON, no markdown, no preamble.

URL: {url}
GOOGLE SNIPPETS: {snippet_text}
PAGE TEXT: {page_text}

Return JSON exactly in this shape:
{{
  "prospect_name": "person's full name, or company name if this is a company page",
  "job_title": "their role, best guess, empty string if unknown",
  "company": "company name",
  "stage": "one of: Early-stage Startup, Series A SaaS, Growth-stage SaaS, Bootstrapped SaaS — best guess",
  "pain_point": "one likely operational pain point, short phrase, best guess based on industry/role"
}}

If information is genuinely unavailable, use empty string rather than inventing specifics."""
    ...
    try:
        ...
        extracted = {"prospect_name": "", "job_title": "", "company": "", "stage": "Early-stage Startup", "pain_point": ""}

    state["prospect_name"] = extracted.get("prospect_name", "")
    state["job_title"] = extracted.get("job_title", "")
    state["company"] = extracted.get("company", "")
    state["stage"] = extracted.get("stage", "Early-stage Startup")
    state["pain_point"] = extracted.get("pain_point", "")
    return state
...
lead_url = st.text_input(
    ...
    placeholder="e.g. https://linkedin.com/in/rahulsharma or https://razorpay.com"
)
...
if st.button("🚀 Run AI SDR Agent", use_container_width=True):
    if not lead_url or not offer:
        st.error("Please provide a LinkedIn/website URL and your Offer.")
    else:
        graph = build_graph()

        col1, col2, col3, col4, col5, col6 = st.columns(6)
        with col1: extraction_status = st.empty()
        with col2: research_status = st.empty()
        with col3: outreach_status = st.empty()
        with col4: followup_status = st.empty()
        with col5: scoring_status = st.empty()
        with col6: crm_status = st.empty()

        extraction_status.info("🌐 Extracting...")
        ...
        with st.expander("🌐 Auto-extracted lead info"):
            st.write(f"**Name:** {result.get('prospect_name','—')}")
            st.write(f"**Title:** {result.get('job_title','—')}")
            st.write(f"**Company:** {result.get('company','—')}")
            st.write(f"**Stage:** {result.get('stage','—')}")
            st.write(f"**Pain point:** {result.get('pain_point','—')}")
        ...
        if result.get("lead_score") is not None:
            ...
            render_lead_card(result.get("prospect_name",""), result.get("company",""), score, category, reason)
